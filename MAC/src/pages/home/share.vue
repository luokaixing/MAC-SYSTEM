  ...
  <div class='_container_'
    ...
    <v-contextmenu ref="contextmenu"
      ...
      <v-contextmenu-submenu :title="LAN.Watch.toUpperCase()">
        <v-contextmenu-item @click="isColumn=true">{{LAN.List.toUpperCase()}}</v-contextmenu-item>
        <v-contextmenu-item @click="isColumn=false">{{LAN.Thumbnails.toUpperCase()}}</v-contextmenu-item>
    ...
    <share-top ref='shareTop'
               :isColumn.sync='isColumn'
               :allList='allList'
               :checkList="checkList"
               :shareList="staticList"
               @emptyCheckList='emptyCheckList'></share-top>
    ...
    <create-pro :title.sync='LAN.EnterPassword'
                v-if="showPassword">
      <el-input v-model="password"
                autofocus
                class='normal-input'
                @keydown.13='submitPass'
                :placeholder="LAN.EnterPassword"></el-input>
      <div class='ok'
           @click="submitPass">{{LAN.OK.toUpperCase()}}</div>
    </create-pro>
...
// 头部按钮的组件
import shareTop from "../shareComp/shareTop";
// 头部面包屑以及表头
...
export default {
  ...
  watch: {
    checkList(newVal) {
      this.checkAll = newVal.length === this.list.length && newVal.length !== 0;
    },
    staticList: {
      immediate: true,
      handler(newVal) {
  ...
  methods: {
    ...
    async submitPass() {
      ...
      if (res.data.state === "error") {
        ...
        this.$store.dispatch("file/preFixFileList", this.staticList);
        this.showPassword = false;
        // this.$store.dispatch("app/triggler")
      }
    },
    updateCheckAll(val) {
      this.checkAll = val;
    ...
    pan(e) {
      ...
      for (let index = 0; index < folders.length; index++) {
        ...
        if (
          ...
          if (!this.tmpList.includes(+fold.dataset.id)) {
